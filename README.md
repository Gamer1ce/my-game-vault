...
点击没有云端副本的本机视频时，网页会直接使用清单中的只读媒体地址，不再额外请求一次播放地址接口。播放器根据实测缓存速度准备 8–15 秒内容即可开始播放，并始终按“当前播放位置之后的缓存”计算进度；暂停预读超过 12 秒时会允许沿用已有缓存尝试播放。点击“立即播放”只启动同一个播放器，不改写媒体地址、不调用重新加载，已经读取的数据会原样保留。码率高于公网持续上行速度的原画仍需要更长等待，或改用对象存储承载。
