...
点击视频并完成选路后，播放器会显示“播放原画”按钮，并通过浏览器原生加载器预加载。缓存进度只计算 `video.buffered` 中当前位置连续可播放的数据，不把独立 fetch 或 HTTP 缓存算作可播放缓存。播放中缓冲耗尽时，会暂停并积累 12 秒有效缓存再继续；反复卡顿时逐步增加到最多 30 秒。可以点击“使用现有缓存继续”或“暂停等待”。如果手机浏览器暂停了后台加载，会明确提示并保留手动继续按钮，不会无限承诺预缓存。20 秒未起播时仍提供重试入口。打开视频期间暂停背景故障效果和弹幕动画，关闭后恢复，不改变页面设计或视频画质。
