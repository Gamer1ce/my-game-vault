...
若 Cloudflare 免费线路把国内访问调度到境外节点，家庭 50 Mbps 上行也可能在单条高延迟连接上只跑到数 Mbps。项目因此支持可选的 **DNS-only IPv6 直连媒体线路** 和 **Azure 镜像线路**：页面会并行读取每条线路 512 KiB 的 Range 样本，以持续吞吐量而不只是首包延迟选路。结果仅保存在当前会话且 10 分钟过期，浏览器报告网络变更时会立即废弃，避免手机从 Wi-Fi 切到 5G 后继续沿用旧线路。不支持 IPv6、证书校验失败或直连中断时会自动尝试镜像，最后保留当前网站的 Cloudflare 线路。直连配置方式是在私密文件 `data/direct-media-origin.txt` 写入一个 HTTPS Origin，或设置 `DIRECT_MEDIA_ORIGIN`，例如：
