...
点击没有云端副本的本机视频时，网页会直接使用清单中的只读媒体地址，不再额外请求一次播放地址接口。播放器先测量线路能在一秒内下载多少秒原画：速度高于码率时迅速起播，速度低于码率时则按剩余时长扩大起播缓存，最多先准备约 120 秒，以长一些的等待换取连续播放。开始播放后会按线路速度维持约 60、90 或 120 秒读前量：浏览器自己的媒体加载器工作时完全让路，进入空闲状态后才以 8 MiB 小 Range 继续预热后续片段；一旦原生加载恢复、缓存不足、暂停、跳转或关闭播放器，后台预读会立即停止。点击“立即播放”仍只启动同一个播放器，不改写媒体地址、不调用重新加载，已经读取的数据会原样保留。
