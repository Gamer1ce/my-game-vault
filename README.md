...
每年 8 月 16 日（以上海时区为准），留言框会用轻微的青红边框和“今天，试着留下一句只属于寿星的祝福……”作为线索。成功发送包含“生日快乐”的留言会触发生日彩蛋：服务器会为这条已保存的留言签发独立回执编号，网页随后显示可截图的感谢语与蛋糕领取说明；其他日期、未成功发送的留言和不含关键词的内容都不会触发。维护者可在本机打开 `http://127.0.0.1:4173/?birthday-preview=0816` 同时预览线索与回执排版，公网域名不会响应这个预览参数。
