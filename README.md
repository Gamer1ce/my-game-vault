...
若 Cloudflare 免费线路把国内访问调度到境外节点，家庭 50 Mbps 上行也可能在单条高延迟连接上只跑到数 Mbps。项目因此支持可选的 **DNS-only IPv6 直连媒体线路** 和 **Azure 镜像线路**：每次打开视频时，页面都会并行读取每条线路 512 KiB 的 Range 样本，以持续吞吐量而不只是首包延迟选路；不再复用上一个视频的结果。选定后，高延迟或索引读取慢只会增加缓存等待，不会被误判为故障而反复换线；只有浏览器返回明确的媒体错误时才会尝试下一条。不支持 IPv6、证书校验失败或直连中断时会自动尝试镜像，最后保留当前网站的 Cloudflare 线路。直连配置方式是在私密文件 `data/direct-media-origin.txt` 写入一个 HTTPS Origin，或设置 `DIRECT_MEDIA_ORIGIN`，例如：
