...
把希望公开展示的文件直接放进这个文件夹，再刷新网站即可。“精彩时刻”会在每次打开页面时随机排列截图；视频会先按 32、96、256 MiB 三档体积分组，再在每档内部随机，使首屏保留变化的同时优先展示更容易流畅播放的文件。切换类型、显示更多或折叠时会保持本次页面已经生成的顺序。每类媒体独立保留“显示更多”和“折叠”的展示进度；文件名会作为标题显示，例如 `赛博朋克2077_荒坂塔.mp4` 会显示为“赛博朋克2077 荒坂塔”。目前支持：
