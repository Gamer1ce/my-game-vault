...
点击没有云端副本的本机视频时，网页会直接使用清单中的只读媒体地址，不再额外请求一次播放地址接口。播放器根据实测缓存速度准备 8–15 秒内容即可开始播放；这个数字只负责起播，不是播放期间的缓存上限。开始播放后会按线路速度维持约 60、90 或 120 秒读前量：浏览器自己的媒体加载器工作时完全让路，进入空闲状态后才以 8 MiB 小 Range 继续预热后续片段；一旦原生加载恢复、缓存不足、暂停、跳转或关闭播放器，后台预读会立即停止。点击“立即播放”仍只启动同一个播放器，不改写媒体地址、不调用重新加载，已经读取的数据会原样保留。

若 Cloudflare 免费线路把国内访问调度到境外节点，家庭 50 Mbps 上行也可能在单条高延迟连接上只跑到数 Mbps。项目因此支持一个可选的 **DNS-only IPv6 直连媒体线路**：页面会并行读取每条线路 96 KiB 测速样本，并在 2.5 秒内选出实测更快的一条；选择结果在当前浏览器保存 24 小时，避免每次播放重复测速。不支持 IPv6、证书校验失败或直连中断时仍保留当前网站线路作为自动回退。配置方式是在私密文件 `data/direct-media-origin.txt` 写入一个 HTTPS Origin，或设置 `DIRECT_MEDIA_ORIGIN`，例如：
