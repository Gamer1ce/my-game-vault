...
> 新增独立的[朋友语音频道](docs/voice-community.md)：用户名与密码注册、三个最多 6 人的点对点语音房间，以及可定制的个人面板。可用一句话让本机 CPA 的 `grok-4.6` 设计配色、布局、背景和字体，先预览再应用；密钥仅由服务器使用，并设每日限额。入口位于主站导航栏；Azure 备用站保持不变。不开启录音或转写，不使用 Azure 语音中继；受网络条件影响，点对点连接不保证所有移动网络均可互通。

需要通过同一域名远程调用本机 CPA，可参考[私人 CPA 接入与安全说明](docs/private-cpa.md)。该入口只开放鉴权后的 AI 接口，不公开管理后台，也不把密钥放进网页。

媒体库递归读取配置目录中的子文件夹（最多 32 层），支持 `PS5/CREATE/Video Clips/游戏名/录像.webm`、`SWITCH/游戏名/录像.mp4` 等结构。游戏文件夹名也会用于分类，纯时间戳录像无需改名。文件以完整相对路径区分，不同文件夹里的同名录像不会互相覆盖。隐藏目录、废纸篓、系统目录、符号链接及路径穿越请求均被排除；请只将愿意公开的媒体放进配置目录。根目录原有链接保持兼容。新增媒体在刷新页面或“刷新媒体库”后显示；Azure 镜像仍按已有同步周期更新。

Azure 镜像同步会预留至少 2 GiB 磁盘空间。备用机剩余空间过低时暂停同步，保留现有数据库和视频；媒体总量超过可容纳容量时不再继续上传，也不会自动删除远端视频腾空间。此时主站仍可通过家庭 IPv6 播放全部本机媒体，但备用站只能展示已同步的部分，需要先释放空间或扩容后才能补齐。
