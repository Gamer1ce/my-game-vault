...
当前部署采用[用户登录与独立视频空间](docs/user-media.md)：访客只看公开硬盘的精彩时刻，指定视频账号登录后进入独立片库。管理员仍使用原账号管理，个人视频账号无管理员权限。Azure 备用站已退役，主站已关闭相关同步、轮询和镜像播放线路；下文 Azure 方案仅供历史部署参考，不代表当前仍在运行。

私人视频支持受登录保护的家庭 IPv6 直连与兼容线路回退。短视频可完整缓存在内存后原画播放，大视频使用流式缓冲；关闭后释放缓存，不生成公开分享链接。

> 新增独立的[朋友语音频道](docs/voice-community.md)：用户名与密码注册、三个最多 6 人的点对点语音房间，以及可定制的个人面板。可用一句话让本机 CPA 的 `grok-4.6` 设计配色、布局、背景和字体，先预览再应用；密钥仅由服务器使用，并设每日限额。入口位于主站导航栏。不开启录音或转写，不使用 Azure 语音中继；受网络条件影响，点对点连接不保证所有移动网络均可互通。

需要通过同一域名远程调用本机 CPA，可参考[私人 CPA 接入与安全说明](docs/private-cpa.md)。该入口只开放鉴权后的 AI 接口，不公开管理后台，也不把密钥放进网页。

媒体库递归读取配置目录中的子文件夹（最多 32 层），支持 `PS5/CREATE/Video Clips/游戏名/录像.webm`、`SWITCH/游戏名/录像.mp4` 等结构。游戏文件夹名也会用于分类，纯时间戳录像无需改名。文件以完整相对路径区分，不同文件夹里的同名录像不会互相覆盖。隐藏目录、废纸篓、系统目录、符号链接及路径穿越请求均被排除；请只将愿意公开的媒体放进公开目录。根目录原有链接保持兼容。新增媒体在刷新页面或“刷新媒体库”后显示；私人视频只在登录后的独立页面读取。

历史 Azure 镜像脚本保留在源码中，当前不会自动执行。若将来重新启用，需重新配置服务器、凭据、目录与容量，不要把个人账号数据库或私有视频同步到公开镜像。

精彩时刻支持按游戏分类、搜索文件名或日期，以及随机、文件更新时间、名称、大小排序。切换筛选后仍先展示 4 个，点击“显示更多”每次追加 8 个。

- 新视频仍放进原媒体文件夹；打开页面或点击“刷新媒体库”，便会自动识别并归类，无须额外运行任务。单次本地扫描最多展示 5,000 个媒体文件。
- 识别依据是录屏文件名里的游戏名、已有游戏库名称和内置别名，不上传视频分析，不额外收取 AI 费用。例如 `THE FINALS-2026_09_11.mp4` 与 `The Finals 2026.09.11.DVR.mp4` 会归在一起。也可以使用 `[游戏名称] 任意说明.mp4` 的命名形式。
- 只有数字时间戳、`Wingdk` 等通用进程名的文件会进入“未分类”，不会凭空猜测内容。进入主站管理员模式后，点击卡片的“修改分类”即可指定游戏；勾选同前缀规则，现有和以后同样命名的视频都会沿用此分类。单文件指定优先于前缀规则。
- 留空保存恢复该文件的自动分类；同时勾选同前缀选项可移除相应规则。访客只能查看和筛选，不能修改分类。
- 分类和规则保存于私密 `games.db`，随现有数据库镜像任务同步到备用站；请在主站修改。分类不会移动、重命名、转码或删除外置硬盘里的原文件，原画播放和 8443 直连保持不变。
...
- **为自托管公网优化**：安全状态与首页数据并行读取，长游戏库按批增量渲染，活动、连接和媒体板块在接近视口时才加载；截图列表只取缩略图、点开后再读取原图，版本化静态资源则可长期复用浏览器缓存。
...
首页完成 `PRESS START` 接入序列后会进入 `/minecraft.html`。该页面每 5 秒读取一次只读状态接口 `/api/minecraft/status`，显示服务器在线状态、玩家人数、延迟、版本、TPS、MSPT、运行时间和 JVM 内存；右上角“返回原网站”按钮会回到游戏档案首页。页面还提供玩家出入日志：Forge 登录/退出事件会先追加到本机只读事件日志，再幂等写入 SQLite，记录玩家名称、加入时间、退出时间和单次在线时长，网页关闭或网站重启都不会丢失。页面装饰采用纯 CSS 末地水晶：双层玻璃笼与粉紫核心分别旋转，水晶整体持续上下漂浮，移动端会隐藏装饰以免挤压状态信息。
...
该脚本会把两个只读 Java Agent 接入当前 Forge 1.20.1 进程：原有 Agent 把 `status.json` 写入 `~/.local/share/game-vault-minecraft/`，事件 Agent 直接监听 Forge 的玩家登录与退出事件，并按每次 JVM 运行写入独立的 `events/*.ndjson`。事件 Agent 还会更新独立心跳；网页只有在监听器完整注册、心跳新鲜且事件序号连续时才显示 `LIVE`，否则保留已有档案并标为 `ARCHIVE` 或 `DEGRADED`。脚本也会把两条 `-javaagent` 参数幂等写入服务端的 `user_jvm_args.txt`，以后重启 Minecraft 仍会自动采集。Compose 只读挂载这个目录，不会向 Minecraft 进程发送管理命令。

玩家出入日志是公开页面的一部分，只返回玩家名称与时间，不返回 UUID、IP 或坐标。安装不会从旧版服务端日志追溯历史，会从事件 Agent 成功接入时开始记录；首次热接入时已经在线的玩家只能标记为“接入前已在线”，不会伪造加入时间。异常崩溃没有正常退出事件时，页面只显示“早于下一次启动时间”，不伪造退出时刻，也不把停机时间算进在线时长。正式公开网站前，应确认玩家已经知道游戏昵称会展示给拿到网址的访客。公网连接地址也属于公开信息，制作自己的版本时务必替换示例地址，并确认路由器与系统防火墙只开放确实需要的游戏端口。
...
每年 8 月 16 日（以上海时区为准），留言框会显示一次隐藏线索：页面打开后保持普通文案 5 秒，经过 0.65 秒故障动画切换为“今天，试着留下一句只属于寿星的祝福……”，停留 15 秒后再经过故障动画恢复，随后保持普通状态且不再循环。成功发送包含“生日快乐”的留言会触发生日彩蛋：服务器会为这条已保存的留言签发独立回执编号，网页随后显示可截图的感谢语与蛋糕领取说明；其他日期、未成功发送的留言和不含关键词的内容都不会触发。维护者可在本机打开 `http://127.0.0.1:4173/?birthday-preview=0816` 同时预览线索与回执排版，公网域名不会响应这个预览参数。
...
把希望公开展示的文件直接放进这个文件夹，再刷新网站即可。“精彩时刻”会在每次打开页面时随机排列截图；不超过 96 MiB 的视频会合并成首屏候选池并整体随机，因此首页不会总是先出现十几 MiB 的小视频。更大的视频仍按体积分档后置，避免首屏同时加载多路高码率内容。切换类型、显示更多或折叠时会保持本次页面已经生成的顺序。每类媒体独立保留“显示更多”和“折叠”的展示进度；文件名会作为标题显示，例如 `赛博朋克2077_荒坂塔.mp4` 会显示为“赛博朋克2077 荒坂塔”。目前支持：
...
网站通过只读接口 `GET /api/highlights` 返回媒体清单，通过 `/media/highlights/文件名?v=修改时间` 提供图片和支持断点播放的视频。版本参数与强 ETag 让浏览器/CDN 可以长期缓存不变的文件；替换同名文件后修改时间会生成新地址，不会继续读到旧内容。它没有网页上传、覆盖或删除接口；访客只能查看。要移除内容，直接在本机删除对应文件并刷新页面。

页面最底部有一个低调的“运行中 / 休眠中”展示按钮，任何访客都可以单击切换，其他已打开的页面会在 30 秒内更新状态。它只保存和显示文字状态，不控制任何网站功能：无论显示“运行中”还是“休眠中”，精彩时刻清单、封面、截图、视频 Range、云端播放地址、游戏档案、留言和定时同步都会照常工作。状态保存在私密的 `data/runtime/media-power.json` 中，Docker 重启后仍会保持。
...
视频卡片不会再读取原视频。服务器首次需要某张封面时会使用 FFmpeg 提取一张小型 JPEG 并缓存在 `data/highlight-posters/`；截图卡片也使用同尺寸缩略图，只有点开查看完整画面时才读取原图。后续列表只请求这些小图，因此不会出现多份原始媒体同时抢占公网带宽。Docker 镜像已经包含生成缩略图所需的 FFmpeg，外置硬盘仍保持只读。

**视频优先加载**：打开视频时，自动暂停当前网页的留言/状态轮询、非媒体数据读取、游戏卡片追加，以及尚未完成的背景海报和缩略图加载；关闭视频后恢复。已加载图片保留，不影响视频请求、每小时后台同步或管理员写入，也不增加访客面板。这只能减少本站的资源竞争，不能独占设备带宽或控制其他 App。实现与边界见[视频优先说明](docs/lossless-video-streaming.md#视频优先)。

点击视频并完成选路后，访客只看到视频、简短状态和必要的播放/换线按钮；管理员可展开“播放诊断”查看测速、码率和有效缓存。缓存进度只计算 `video.buffered` 中当前位置连续可播放的数据，不把独立 fetch 或 HTTP 缓存算作可播放缓存。普通 MP4 由浏览器持续加载，不主动暂停等待固定的 12 秒缓存，以免手机在约 4 秒预加载上限处一直等待。高码率 H.264/AAC 原片可以使用[原画分段播放](docs/lossless-video-streaming.md)：通过本机脚本无损重封装，播放器按约 2 秒小段加载，目标向前缓存约 16 秒，设备限制时沿用已有缓存继续；不转码、不降低分辨率或帧率。20 秒未起播时仍提供重试入口。打开视频期间暂停背景故障效果和弹幕动画，关闭后恢复。

管理员播放诊断中显示原片平均码率和实际可播放缓存。短样本网速不是持续带宽：例如 5 分钟、3.4 GB 的原片平均需要约 91 Mbps，并且瞬时码率可能更高；测速 315 Mbps 不能保证移动网络全程稳定。缓存充足但仍掉帧时，页面会提示可能存在设备解码或省电模式限制。所有上述优化均不重新编码，不修改原视频。实际体验仍取决于访问者链路和设备，不能以服务器本机测速代替外地 5G 测试。

项目支持可选的 **DNS-only IPv6 直连媒体线路** 和 **Azure 镜像线路**：每次打开视频时，页面并行读取每条线路 512 KiB 文件头 Range 样本和 64 KiB 文件尾，校验 Range 可用性并参考短样本速率排序。检测请求绕过浏览器缓存，但仍可能命中上游缓存，因此界面明确标为“短样本（非持续网速）”。选定后，慢速本身不会触发反复换线；明确的媒体错误才尝试下一条，也可手动换线，切换时保留播放位置。不支持 IPv6 时保留镜像和网站兼容线路。直连配置是在私密文件 `data/direct-media-origin.txt` 写入 HTTPS Origin，或设置 `DIRECT_MEDIA_ORIGIN`，例如：
...
若运营商阻止境外 ACME 服务器直接访问家庭 IPv6，可执行 `npm run media:direct-cert`，使用钥匙串中现有的 Cloudflare DNS Token 通过 DNS-01 签发或续期证书。项目所有者的 Mac 已安装每日 LaunchAgent；脚本仅在证书剩余不超过 30 天时续期，证书内容发生变化后才重启 Caddy。Cloudflare Token 只作为子进程环境变量传入，不会写入仓库、日志或证书目录。

MP4/MOV 可以执行一次无损 Fast Start：`npm run media:faststart`。命令只通过 FFmpeg `-c copy` 重排容器索引，不重新编码视频或音频；已经优化的文件会跳过，输出通过结构校验后才原位替换。优化后的修改时间会更新，以刷新浏览器缓存并触发镜像同步。每次新增录像后可再次运行。首次运行前可用 `node scripts/optimize-highlight-mp4.mjs --dry-run` 查看需要处理的文件。
...
- 首次播放时，浏览器会同时对可用线路读取 96 KiB 的 Range 样本，最多等待 2.5 秒，选择实测速率更高的一条。
- 选择结果在当前浏览器保存 24 小时；清理站点数据后会重新测速。
...
连接成功后会立即同步一次。服务启动后也会自动检查已连接的平台，持续运行时每小时再次同步；四个平台完成后会在同一轮检查 RAWG/Metacritic，为刚出现的新游戏补全 MC 评分。管理员还可使用页面底部的“立即同步游戏数据”一次更新全部已连接平台和评分，或在“数据来源”中单独同步某个平台。手动同步和定时同步重叠时只会执行同一轮任务，避免重复请求平台接口。
...
Mac 端网站服务会在启动约 45 秒后执行一次 Azure 同步，此后每 60 分钟执行一次；若上一次媒体传输尚未结束，新一轮会自动跳过。Docker 模式的每轮任务只替换 `games.db` 的一致性快照并同步精彩时刻，不会覆盖 Azure 的 `community.db`，因此访客留言、点赞与反馈会在游戏数据更新后继续保留。同步脚本默认要求本地快照至少包含 1 款可展示游戏；如果工作区误挂载到空数据库，它会中止而不是清空公网档案。可通过 `AZURE_BACKUP_MIN_VISIBLE_GAMES` 提高最低安全数量，只有明确需要发布空档案时才应设为 `0`。它也不会在 Azure 构建程序；源码更新通过前述独立镜像发布流程完成。平台凭据、本机配置、Git 历史和忽略文件不会上传。首次媒体同步约 12GB，速度受 Mac 上行带宽影响，可能持续数小时；rsync 会保留未完成分片，任务再次运行时继续传输。只有完整文件落盘后才会出现在精彩时刻清单中。媒体目录采用镜像模式：新增和修改会上传，删除或重命名会在下一次成功同步结束时清理 Azure 上的旧文件；移动硬盘未挂载时不会执行媒体同步或远端删除。Azure VM、Premium SSD、公网 IPv4 和出站流量可能消耗学生订阅额度，长期运行时应在 Azure 成本管理中设置预算提醒。
...
这个方案不需要 Azure 订阅。OpenXBL 是非微软官方的第三方网关，免费版有请求频率限制，且部分游戏不会发布 `MinutesPlayed`，因此可能出现有游戏记录但时长缺失的情况。程序把已经确认的累计分钟作为只增不减的基线：后续接口临时缺失 `MinutesPlayed` 或返回 0 时会保留旧值，不会让游戏从公开统计中消失。标题汇总没有返回总成就数时，程序会对有时长的游戏调用 OpenXBL 的单游戏成就列表补全分母，并缓存已取得的总数，避免每次同步重复消耗免费额度。
...
评分连接成功后，每次启动、每小时自动同步和底部“一键同步”都会在游戏平台更新结束后执行评分补全。新游戏无需再单独点击 RAWG 的“立即同步”；没有评分的记录会在 30 天后重新检查，以便收录刚发布的媒体评分。
...
- “最近两周”图表读取同一份逐日数据库：每天一列并按平台颜色叠加；带 `≈` 的日期包含同步检测增量，不带 `≈` 的日期全部来自平台逐日记录。页面右下方常驻“顶部”和“底部”两个悬浮入口；“底部”会避开游戏库的自动分批追加并一次跳到页面真实末端，移动端采用更紧凑的按钮尺寸以减少遮挡。
...
| Xbox 同步成功却没有时长 | 不是每款游戏都向 Xbox 统计接口发布 `MinutesPlayed`，OpenXBL 也可能在某一轮同步中临时漏回部分统计 | 首次没有可靠时长的游戏仍不展示；已经记录过的累计时长只增不减，后续缺失或 0 值不会覆盖旧基线，也不会把游戏从页面隐藏 |
...
首页「AI 用量」按钮使用原版 KEEPER 界面查看只读用量；访客仅能读取经过清洗的统计，网站管理员登录后才能查看客户端密钥、连接地址和管理视图。原始管理端口不直接公开。[接入与安全说明](docs/ai-usage.md)。
