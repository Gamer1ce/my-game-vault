...
点击视频并完成选路后，播放器会立即显示可用的“播放原画”按钮。点击按钮会直接调用浏览器播放功能，兼容手机限制自动播放或后台预加载的情况。请求播放与真正出现画面分别计时；20 秒仍未起播时会显示“重试播放”和换线入口，不会一直停留在读取索引的提示上。换线时继续保留播放位置，慢速本身不会触发自动换线。播放中的读前请求只在原生加载器空闲时进行；关闭播放器会停止播放和后台预读。实际流畅程度仍取决于手机到所选节点的吞吐量与原视频码率。

若 Cloudflare 免费线路把国内访问调度到境外节点，家庭 50 Mbps 上行也可能在单条高延迟连接上只跑到数 Mbps。项目因此支持可选的 **DNS-only IPv6 直连媒体线路** 和 **Azure 镜像线路**：每次打开视频时，页面都会并行读取每条线路 512 KiB 的文件头 Range 样本，再读取 64 KiB 文件尾验证索引跳转，以持续吞吐量和真实播放能力排序；不再复用上一个视频的结果。选定后，高延迟或索引读取慢只会增加缓存等待，不会被误判为故障而反复换线；只有浏览器返回明确的媒体错误时才会尝试下一条。备用线路也按实测速度排列，切换后会恢复当前播放位置；用户也可随时点击“换条线路”手动切换。不支持 IPv6、证书校验失败或直连中断时会自动尝试镜像，最后保留当前网站的 Cloudflare 线路。直连配置方式是在私密文件 `data/direct-media-origin.txt` 写入一个 HTTPS Origin，或设置 `DIRECT_MEDIA_ORIGIN`，例如：
