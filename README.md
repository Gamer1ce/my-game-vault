...
页面最底部有一个低调的“运行中 / 休眠中”按钮，任何访客都可以单击切换精彩时刻媒体服务，其他已打开的页面会在 30 秒内更新状态。休眠不会关闭网站、游戏档案、留言或定时同步，只会停止新的媒体清单、封面、截图、视频 Range 与云端播放地址请求；正在传输的有限分段结束后不会再读取下一段。已经缓存到访客设备里的片段仍可能在本地继续播放，但不会再次消耗家庭带宽。状态保存在私密的 `data/runtime/media-power.json` 中，Docker 重启后仍会保持。服务器执行同源校验、每 IP 每分钟最多 12 次以及全局 5 秒冷却，避免误触或脚本高速反复切换。
...
视频卡片不会再读取原视频。服务器首次需要某张封面时会使用 FFmpeg 提取一张小型 JPEG 并缓存在 `data/highlight-posters/`；后续页面只请求这张静态图，因此不会出现四个视频同时抢占公网带宽。Docker 镜像已经包含生成封面所需的 FFmpeg，外置硬盘仍保持只读。
...
若 Cloudflare 免费线路把国内访问调度到境外节点，家庭 50 Mbps 上行也可能在单条高延迟连接上只跑到数 Mbps。项目因此支持一个可选的 **DNS-only IPv6 直连媒体线路**：页面会并行读取每条线路 256 KiB 测速样本，优先使用实际更快的一条；不支持 IPv6、证书校验失败或直连中断时仍保留当前网站线路作为自动回退。配置方式是在私密文件 `data/direct-media-origin.txt` 写入一个 HTTPS Origin，或设置 `DIRECT_MEDIA_ORIGIN`，例如：
