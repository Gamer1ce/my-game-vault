...
button returned of (display dialog "选择精彩时刻媒体库的位置。使用外置硬盘可以节省电脑空间；硬盘断开时网站只会暂时隐藏媒体。" with title "游迹" buttons {"取消", "使用项目默认文件夹", "选择外置硬盘文件夹"} default button "选择外置硬盘文件夹" cancel button "取消")
APPLESCRIPT
)" || exit 0
...
if curl -fsS "http://localhost:4173/api/highlights" >/dev/null 2>&1; then
  ...
  echo "下次双击“启动游迹.command”时会读取这个目录。"
fi
