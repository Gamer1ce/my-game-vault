...
export function createBaiduProxyWorker({ fetchImpl = fetch, now = () => Date.now() } = {}) {
  return {
    async fetch(request, env) {
      if (request.method === "OPTIONS") {
        return new Response(null, { status: 204, headers: responseHeaders(new Response(), env.ALLOWED_ORIGIN) });
      }
      if (request.method !== "GET") return new Response("Method Not Allowed", { status: 405 });
      if (!requestOriginAllowed(request, env.ALLOWED_ORIGIN)) return new Response("Forbidden", { status: 403 });
      const parts = new URL(request.url).pathname.split("/").filter(Boolean);
      if (parts[0] !== "v1" || !parts[1]) return new Response("Not Found", { status: 404 });
      let payload;
      try {
        payload = await decryptPayload(parts[1], env.PROXY_KEY);
      } catch {
      ...
      return new Response(upstream.body, {
        ...
        headers: responseHeaders(upstream, env.ALLOWED_ORIGIN)
      });
    }
  };
}

export default createBaiduProxyWorker();
