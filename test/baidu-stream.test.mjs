...
test("阿里云ESA根地址显示媒体节点状态并返回主站入口", async () => {
  const worker = createBaiduEsaProxy({
    bundledEnvironment: { PROXY_KEY: keyText, ALLOWED_ORIGIN: "https://games.example.com" }
  });
  const response = await worker.fetch(new Request("https://media-cn.example.com/"));
  assert.equal(response.status, 200);
  assert.equal(response.headers.get("X-Media-Edge"), "aliyun-esa");
  assert.match(await response.text(), /媒体节点在线/);
  assert.match(await (await worker.fetch(new Request("https://media-cn.example.com/"))).text(), /https:\/\/games\.example\.com/);
});
